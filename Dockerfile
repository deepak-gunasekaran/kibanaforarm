...
# set the Elastic Server Address at variable $ELASTIC_SEARCH
...
RUN set -x \
    ...
    && sed -ri "s!^(\#\s*)?(elasticsearch\.url:).*!\2 'http://${ELASTIC_SEARCH}:9200'!" /opt/kibana/config/kibana.yml \
    && grep -q '${ELASTIC_SEARCH}:9200' /opt/kibana/config/kibana.yml
